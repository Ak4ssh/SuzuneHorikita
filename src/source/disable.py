...
from src.source.helper_funcs.handlers import CMD_STARTERS, SpamChecker
from src.source.helper_funcs.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    from src.source.sql import disable_sql as sql
                    ...
                    if len(fst_word) > 1 and any(
                        ...
                        if filter_result:
                            ...
                            if sql.is_command_disabled(chat.id, command[0].lower()):
                                ...
                                is_disabled = command[
                                    0
                                ] in ADMIN_CMDS and is_TheRiZoeL(chat, user.id)
                                if not is_disabled:
                                    return None
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "src.source." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            enable_module = "src.source." + args[0].rsplit(".", 1)[0]
