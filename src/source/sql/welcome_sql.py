...
Cleanhelp1etting.__table__.create(checkfirst=True)
...
def clean_service(chat_id: Union[str, int]) -> bool:
    try:
        chat_setting = SESSION.query(Cleanhelp1etting).get(str(chat_id))
        if chat_setting:
...
def set_clean_service(chat_id: Union[int, str], setting: bool):
    with CS_LOCK:
        chat_setting = SESSION.query(Cleanhelp1etting).get(str(chat_id))
        if not chat_setting:
            chat_setting = Cleanhelp1etting(chat_id)
