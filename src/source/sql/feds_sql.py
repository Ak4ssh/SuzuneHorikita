...
class Federations(BASE):
    ...
    def __init__(self, Owner, fed_name, fed_id, fed_rules, fed_log, fed_users):
        self.Owner = Owner
...
def get_TheRiZoeL_fed_name(user_id):
    user_feds = []
...
def get_TheRiZoeL_fed_full(user_id):
    user_feds = []
    for f in FEDERATION_BYFEDID:
        if int(user_id) in ast.literal_eval(
            ast.literal_eval(FEDERATION_BYFEDID[f]["fusers"])["members"]
...
def new_fed(Owner, fed_name, fed_id):
    with FEDS_LOCK:
        ...
        fed = Federations(
            ...
            None,
            str({"owner": str(Owner), "members": "[]"}),
        )
        SESSION.add(fed)
        SESSION.commit()
        FEDERATION_BYOWNER[str(Owner)] = {
            "fid": str(fed_id),
            "fname": fed_name,
            "frules": "Rules is not set in this federation.",
            "flog": None,
            "fusers": str({"owner": str(Owner), "members": "[]"}),
        }
...
def rename_fed(fed_id, Owner, newname):
    with FEDS_LOCK:
        ...
        FEDERATION_BYOWNER[str(Owner)]["fname"] = newname
...
def user_demote_fed(fed_id, user_id):
    with FEDS_LOCK:
        ...
        # Set user
        FEDERATION_BYOWNER[str(Owner)]["fusers"] = str(
            {"owner": str(Owner), "members": str(members)},
        )
        ...
        fed = Federations(
            ...
            fed_rules,
            fed_log,
            str({"owner": str(Owner), "members": str(members)}),
        )
        SESSION.merge(fed)
        SESSION.commit()
        return True
...
def set_frules(fed_id, rules):
    with FEDS_LOCK:
        ...
        fed_log = getfed["flog"]
        # Set user
        FEDERATION_BYOWNER[str(Owner)]["frules"] = fed_rules
...
def set_fed_log(fed_id, chat_id):
    with FEDS_LOCK:
        ...
        fed_log = str(chat_id)
        # Set user
        FEDERATION_BYOWNER[str(Owner)]["flog"] = fed_log
...
def __load_all_feds():
    ...
    try:
        ...
        for x in feds:  # remove tuple by ( ,)
            # Fed by Owner
            check = FEDERATION_BYOWNER.get(x.Owner)
            if check is None:
                FEDERATION_BYOWNER[x.Owner] = []
            FEDERATION_BYOWNER[str(x.Owner)] = {
                "fid": str(x.fed_id),
                "fname": x.fed_name,
                "frules": x.fed_rules,
                "flog": x.fed_log,
