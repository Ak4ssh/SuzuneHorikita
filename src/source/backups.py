...
import src.source.sql.notes_sql as sql
from src import dispatcher, LOGGER, Owner, JOIN_LOGGER, SUPPORT_CHAT
from src.__main__ import DATA_IMPORT
from src.source.helper_funcs.chat_status import TheRiZoeL
from src.source.helper_funcs.alternate import typing_action

# from src.source.rules import get_rules
import src.source.sql.rules_sql as rulessql

# from src.source.sql import warns_sql as warnssql
import src.source.sql.blacklist_sql as blacklistsql
from src.source.sql import disable_sql as disabledsql

# from src.source.sql import cust_filters_sql as filtersql
# import src.source.sql.welcome_sql as welcsql
import src.source.sql.locks_sql as locksql
...
def export_data(update, context):
    ...
    with open("src{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("src{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `src-Backup` was specially made for notes.".format(
            chat.title,
            chat_id,
            tgl,
        ),
    ...
    os.remove("src{}.backup".format(chat_id))  # Cleaning file
