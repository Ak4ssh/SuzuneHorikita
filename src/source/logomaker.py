...
from src import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("./src/data/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Suzune Robot](https://t.me/EmiexRobot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./src/data/fonts"):
            os.remove(font_)
...
saxsux = """ This is help menu for logomaker
