...
from src import dispatcher, LOGGER, DRAGONS as SUDO_USERS
...
@typing_action
@Kaguyacmd(command='filters', admin_ok=True)
...
@Kaguyacmd(command='filter', run_async=False)
...
# NOT ASYNC BECAUSE DISPATCHER HANDLER RAISED
@Kaguyacmd(command='stop', run_async=False)
@user_admin(AdminPerms.CAN_CHANGE_INFO)
@typing_action
...
@Kaguyamsg((CustomFilters.has_text & ~Filters.update.edited_message))
...
@Kaguyacmd(command="removeallfilters", filters=Filters.chat_type.groups)
...
@Kaguyacallback(pattern=r"filters_.*")
