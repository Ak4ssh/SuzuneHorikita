...
@dev_plus
@loggable
def gbanreq_btn(update: Update, context: CallbackContext) -> str:
    bot = context.bot
    query = update.callback_query
    chat = update.effective_chat
    user = update.effective_user
    if query.data != "gbanreq_delete":
        splitter = query.data.split("=")
        query_match = splitter[0]
        if query_match == "gbanreq_gb":
            user_id = splitter[1]
            log_message = ""
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton(text="Delete", callback_data="gbanreq_delete"),
                    ]
                ]
            ),
            parse_mode=ParseMode.HTML,
...
"""
GBAN_BUTTON_HANDLER = CallbackQueryHandler(gbanreq_btn, pattern=r"gbanreq_")
...
dispatcher.add_handler(GBAN_BUTTON_HANDLER)
